...
渲染成：**辐射亮度 (radiance)**，虚线下划线，悬停显示定义。
...
辐射亮度:
  en: radiance                     # 必填
  def: 每单位投影面积、每单位立体角… # 可选，悬停显示 + 术语表页
  aka: [亮度]                       # 可选，别名，进搜索索引
  see: physics/optics/radiometry    # 可选，详解这个术语的笔记
```

术语的 `en` 和 `aka` 会拼进该笔记的搜索文本——**搜 "radiance" 能搜到从没写过这个英文单词的中文笔记**。
