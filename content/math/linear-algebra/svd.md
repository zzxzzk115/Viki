...
:::quiz
- ✓ 任何线性变换都等价于「旋转 → 沿坐标轴缩放 → 再旋转」；对任意实矩阵成立，不要求方阵、可逆或对称
- 把矩阵分解为特征向量方向上的独立缩放；只对对称矩阵成立
- 任何线性变换都等价于「缩放 → 剪切 → 投影」；只对满秩矩阵成立
- 把矩阵旋转到对角占优的形态以便快速求逆；只对方阵成立
:::
::::
...
:::quiz
- ✓ 截断 SVD 得到的 A_k 是 Frobenius 范数（及谱范数）下的最优 k 秩近似——不存在更接近 A 的秩 k 矩阵
- 保留前 k 个特征值的截断特征值分解才是最优低秩近似，SVD 只是它的推广
- 截断 SVD 的最优性只对对称半正定矩阵成立，一般矩阵需要迭代求精
- 最优性只在 L1 范数下成立，Frobenius 范数下可能存在更好的秩 k 近似
:::
::::
