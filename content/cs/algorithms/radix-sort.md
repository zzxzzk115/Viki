...
:::quiz
- ✓ 当前位相同的元素，正确顺序由前几轮决定，稳定性保证这个顺序被继承；不稳定则前面所有轮次的工作作废
- 稳定性保证每个桶内部有序，否则收回时还需要对桶做一次额外排序
- 稳定分桶让复杂度保持 O(d(n+k))，不稳定会退化到 O(n²)
- 只有 MSD（从高位开始）基数排序才需要稳定性，LSD 其实不需要
:::
::::
...
:::quiz
- ✓ 前缀和后 count[d] 是数字 d 在输出里的结束位置——倒序遍历配合 count[d] 递减，才能保住相同数字元素的相对顺序（稳定性）
- 倒序遍历是为了缓存友好：输出数组从尾部开始写在硬件上更快
- 这是 Python 的语言限制：range 正序遍历时不允许在循环里修改 count
- 正序倒序都能排对，倒序只是惯例——正确性由前缀和本身保证
:::
::::
